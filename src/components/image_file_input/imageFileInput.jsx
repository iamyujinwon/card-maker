    ...
    <div className='mt-3 flex items-center space-x-3'>
      ...
      {loading && (
        <button
          disabled
          type='button'
          class='py-2 px-6 text-white rounded-lg border border-gray-200 bg-black inline-flex items-center'
        >
          <svg
            role='status'
      ...
      />
      <div>
        {!loading && <div>{name}</div>}
        {!name && !loading && <div>No image chosen</div>}
      </div>
    </div>
  );
});
